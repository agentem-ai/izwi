...
pub async fn execute(args: ServeArgs) -> Result<()> {
    ...
    println!("  Metal GPU:      {}", if args.metal || cfg!(target_os = "macos") { "enabled" } else { "disabled" });
    ...
    println!("  API endpoint: {}", style(format!("http://{}:{}/v1", args.host, args.port)).cyan());
    if !args.no_ui {
        println!("  Web UI:       {}", style(format!("http://{}:{}", args.host, args.port)).cyan());
    }
    ...
    if !status.success() {
        return Err(CliError::Other(format!("Server exited with code: {:?}", status.code())));
    }
