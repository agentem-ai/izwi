...
impl NativeExecutor {
    ...
    pub(super) fn execute_static_parakeet_asr_prefill_requests_with_rows(
        &self,
        requests: &[&EngineCoreRequest],
        scheduled: &[ScheduledRequest],
        rows: Option<&[ReadyQuantum]>,
        ...
        let ordered = scheduled
            ...
            .map(|scheduled| {
                Self::find_request(requests, scheduled).ok_or_else(|| {
                    Error::InferenceError(
                        "static Parakeet prefill row has no request snapshot".into(),
                    )
                })
            })
            .collect::<Result<Vec<_>>>()?;
        let managed = scheduled
            ...
            .collect::<Result<Vec<_>>>()?;
        let outputs = self.parakeet_asr_prefill_batch_with_managed(&ordered, scheduled, managed)?;
        self.finish_scheduled_execution(
            requests,
            scheduled,
            outputs,
