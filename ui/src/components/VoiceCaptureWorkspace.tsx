...
export function VoiceCaptureWorkspace({
  ...
  const handleReferenceReady = useCallback(
    (_audioBase64: string, _transcript: string) => {
      // VoiceClone handles save flows internally; this workspace only tracks readiness signals.
    },
    [],
  );
    ...
    >
      <WorkspacePanel className="p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className={VOICE_ROUTE_SECTION_LABEL_CLASS}>Clone Voice</div>
            <p className="mt-1 text-sm text-[var(--text-secondary)]">
              Upload or record a clean reference with transcript, then save a
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-2">
          ...
          <div className="rounded-xl border border-[var(--border-muted)] bg-[var(--bg-surface-1)] p-4">
            ...
            <div className={VOICE_ROUTE_META_COPY_CLASS}>
              Saved voice profiles appear in your library and can be used in Text
              to Speech.
            </div>
            {savedVoiceId ? (
              <div className="mt-3 flex items-center gap-2 rounded-lg border border-[var(--status-positive-border)] bg-[var(--status-positive-bg)] px-3 py-2 text-xs font-medium text-[var(--status-positive-text)]">
